...
class MyPluginController < ApplicationController
  def index
    ...
    if User.current.logged?
      ...
      unless api_key && workspace_id && user_id
        flash[:error] = "Ozwell configuration missing."
        return
      end

      uri = URI("http://ozwell-service:5000/ozwell/session")
      http = Net::HTTP.new(uri.host, uri.port)
      ...
      begin
        ...
        if response.code.to_i == 200
          @ozwell_login_url = json["loginUrl"]
        else
          flash[:error] = "Ozwell error: #{json["error"]}"
        end
      rescue => e
        flash[:error] = "Ozwell service failed: #{e.message}"
      end
    else
      flash[:error] = "Please log in to use Ozwell integration"
    end
